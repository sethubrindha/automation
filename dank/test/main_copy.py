...
def main():
    ...
    # recent_post = datetime.now() - timedelta(days=60)
    # print("recent_post >>>>>>>>>>>>",recent_post)
    for page_url in selected_pages:
        try:
            ...
            # reel_list = [
            #     post for post in profile.get_posts().page_length(10)
            #     if post.typename in ['GraphVideo', 'GraphSidecar'] and post.likes > 1000
            # ]
            # timer(random.randint(30, 60))

            # Get the date range for the last 2 days

            # Get all reels (posts of type 'video' or 'igtv') from the last 2 days with high views
            temp_reel_list = []
            timer(random.randint(10, 20))
            print("temp_reel_list >>>>>>>",temp_reel_list)
            reels = profile.get_reels()
            print("reels lenght >>>>>>",len(reels))
            for post in reels:
                print("post >>>>>>>>>",post)
                timer(random.randint(3, 10))
                if post.typename in ['GraphVideo', 'GraphSidecar'] and post.is_video:
                    # post_date = post.date
                    # if post_date >= recent_post and post.likes > 1000:
                    if post.likes > 1000:
                        # print('post_date >>>>>>>>>>>>>',post_date)
                        print("post like >>>>>>>>>",post.likes)
                        temp_reel_list.append(post)
            print("temp_reel_list >>>>>>",temp_reel_list)
            reel_list.extend(temp_reel_list)
            pages += f'\n@{profile_name} '
            timer(random.randint(30, 60))
